...
<article id="post-<?php the_ID(); ?>" <?php post_class( array( 'list-article', 'clearfix' ) ); ?>>
	...
	<div class="list-article-content">
		...
		<header class="entry-header">
			<?php the_title( sprintf( '<h2 class="entry-title h2Blue text_align_left"><a href="%s" rel="bookmark">', esc_url( get_permalink() ) ), '</a></h2>' ); ?>
		</header><!-- .entry-header -->
		<div class="entry-excerpt">
			<?php
			//2021/07 ADD 表示する文字数変更ここから
			add_filter( 'excerpt_length', function ( $length ) {
				return 50;//表示する文字数
				}, 999 );
				//2021/07 Add 表示する文字数変更ここまで
				the_excerpt();
			?>
			<?php
				wp_link_pages(
